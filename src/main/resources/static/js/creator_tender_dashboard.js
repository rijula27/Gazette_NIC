...
document.getElementById('editPdfFile').addEventListener('change', (event) => {
...
// send
var sendId;
function sendPublisher(el) {
    sendId = $(el).data('id');
    Swal.fire({
        title: "Confirm Send?",
        text: "Do you want to send this to the publisher?",
        icon: "question",
        showCancelButton: true,
        allowOutsideClick: false,
        allowEscapeKey: false,
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            if (validateTenderForm()) {
                ...
                $('#editModal').modal('show'); // Show modal again if validation fails
            }
        }
    });
}
...
function submitEditForm() {
    ...
    Swal.fire({
        title: "Updating...",
        text: "Please wait while the tender is being updated.",
        allowOutsideClick: false,
        didOpen: () => {
            Swal.showLoading();
            $.ajax({
